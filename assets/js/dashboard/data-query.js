...
async function getData(url = "", keyword = "") {
  ...
  if (response.ok) {
    ...
    json.forEach((queryData, index) => {
      let tableContainer = `<h2 style="font-weight:bold; color: black;">${getTableTitle(index)}</h2>`;
...
function getTableTitle(index) {
  const titles = [
    ...
    "Query 3: Brand and Total Income",
    "Query 4: Brand and Total Sales",
